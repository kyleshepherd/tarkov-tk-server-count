...
import (
	...
	"github.com/kyleshepherd/tarkov-tk-server-count/internal/config"
...
func tarkovtkservercountCmd() *cobra.Command {
	var configPath string
	cmd := &cobra.Command{
		Use:           "tarkov-tk-server-count",
		...
		RunE: tarkovtkservercountRun,
	}
	// Global flags
	pflags := cmd.PersistentFlags()
	pflags.StringVarP(&configPath, "config", "c", "", "path to configuration file (default is $HOME/.config/tarkov-tk-server-count.toml)")
...
// tarkov-tk-server-countRun is executed when the CLI executes
// the tarkov-tk-server-count command
func tarkovtkservercountRun(cmd *cobra.Command, _ []string) error {
	return cmd.Help()
}
