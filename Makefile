# General
CMD      :=
PKG      := github.com/kyleshepherd/tarkov-tk-server-count
...
BIN_NAME   ?= tarkov-tk-server-count-$(shell go env GOOS)-$(shell go env GOARCH)
ifeq ("$(GIT_TAG)","")
	BIN_VERSION = $(GIT_SHA)
endif
BIN_VERSION ?= ${GIT_TAG}
...
# Build a binary
.PHONY: build
build: CMD = ./cmd/tarkov-tk-server-count
